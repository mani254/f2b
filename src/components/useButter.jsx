...
const useButter = (options) => {
	...
	useEffect(() => {
		...
		butterRef.current = butterInstance;
		butterInstance.init(options);

		return () => {
			if (butterInstance.active) {
				butterInstance.cancel();
			}
		};
	}, [options]);

	return butterRef.current;
};
